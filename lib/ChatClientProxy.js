...
class ChatClientProxy {
  ...
  async createChatCompletion(chatSettings, messages) {
    ...
    try {
      return await this.chatClient.generateChatCompletionStream(chatSettings, messages);
    } catch (error) {
      if (error instanceof _apiError.ApiError) {
        return this.handleError(error);
      } else {
        return this.handleError(new _apiError.ApiError('An unexpected error occurred', 500));
      }
    }
  }
  async createChatCompletionNonStreaming(chatSettings, messages) {
    ...
    const stream = await this.chatClient.generateChatCompletionStream(chatSettings, messages);
    return stream.text();
  }
