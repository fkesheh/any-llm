...
class AzureChatClient extends _ChatClientBase.ChatClientBase {
  async initialize(apiKeyValues) {
    ...
    const azureOpenaiApiKey = (0, _serverChatHelpers.checkAndGetEnv)(apiKeyValues, _types.VALID_ENV_KEYS.AZURE_OPENAI_API_KEY);
    const azureOpenaiEndpoint = (0, _serverChatHelpers.checkAndGetEnv)(apiKeyValues, _types.VALID_ENV_KEYS.AZURE_OPENAI_ENDPOINT);
    const deploymentId = (0, _serverChatHelpers.checkAndGetEnv)(apiKeyValues, _types.VALID_ENV_KEYS.AZURE_OPENAI_DEPLOYMENT_ID);
  ...
  async generateChatCompletion(chatSettings, messages) {
    ...
    switch (chatSettings.model.modelId) {
      case 'gpt-3.5-turbo':
        deploymentId = (0, _serverChatHelpers.checkAndGetEnv)(this.apiKeyValues, _types.VALID_ENV_KEYS.AZURE_OPENAI_35_TURBO_ID);
        break;
      case 'gpt-4':
        deploymentId = (0, _serverChatHelpers.checkAndGetEnv)(this.apiKeyValues, _types.VALID_ENV_KEYS.AZURE_OPENAI_4_ID);
        break;
      case 'gpt-4-turbo-preview':
        deploymentId = (0, _serverChatHelpers.checkAndGetEnv)(this.apiKeyValues, _types.VALID_ENV_KEYS.AZURE_OPENAI_4_TURBO_ID);
        break;
      case 'gpt-4-vision-preview':
        deploymentId = (0, _serverChatHelpers.checkAndGetEnv)(this.apiKeyValues, _types.VALID_ENV_KEYS.AZURE_OPENAI_4_TURBO_VISION_ID);
        break;
      default:
  ...
  handleError(error) {
    ...
    if (errorMessage.toLowerCase().includes('api key not found')) {
      errorMessage = 'Azure OpenAI API Key not found. Please set it in your profile settings.';
    } else if (errorMessage.toLowerCase().includes('incorrect api key')) {
      errorMessage = 'Azure OpenAI API Key is incorrect. Please fix it in your profile settings.';
    }
