...
class CohereChatClient extends _ChatClientBase.ChatClientBase {
  async initialize(apiKeyValues) {
    const cohere_api_key = (0, _serverChatHelpers.checkAndGetEnv)(apiKeyValues, _types.VALID_ENV_KEYS.COHERE_API_KEY);
  ...
  handleError(error) {
    ...
    if (errorMessage.toLowerCase().includes('api key not found')) {
      errorMessage = 'Cohere API Key not found. Please set it in your profile settings.';
    } else if (errorMessage.toLowerCase().includes('incorrect api key')) {
      errorMessage = 'Cohere API Key is incorrect. Please fix it in your profile settings.';
    }
