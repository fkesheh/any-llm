...
class GoogleChatClient extends _ChatClientBase.ChatClientBase {
  async initialize(apiKeyValues) {
    const googleGeminiApiKey = (0, _serverChatHelpers.checkAndGetEnv)(
      apiKeyValues,
      _types.VALID_ENV_KEYS.GOOGLE_GEMINI_API_KEY,
    )
    this.googleAI = new _generativeAi.GoogleGenerativeAI(googleGeminiApiKey)
  }
  static toGoogleRole(role) {
    if (
      role === _types.ChatRoles.Assistant ||
      role === _types.ChatRoles.System
    ) {
      return 'model'
    }
    return role
