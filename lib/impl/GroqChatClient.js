...
class GroqChatClient extends _ChatClientBase.ChatClientBase {
  async initialize(apiKeyValues) {
    const grogApiKey = (0, _serverChatHelpers.checkAndGetEnv)(apiKeyValues, _types.VALID_ENV_KEYS.GROQ_API_KEY);
  ...
  handleError(error) {
    ...
    if (errorMessage.toLowerCase().includes('api key not found')) {
      errorMessage = 'Groq API Key not found. Please set it in your profile settings.';
    } else if (errorMessage.toLowerCase().includes('incorrect api key')) {
      errorMessage = 'Groq API Key is incorrect. Please fix it in your profile settings.';
    }
